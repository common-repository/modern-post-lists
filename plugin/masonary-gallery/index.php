...
class modern_post_lists_MasonaryGallery
{
    public $post_type = 'post';
    public $categories = [];
    public $layout_type = 'Masonary Gallery';
    public $name = '';
    public $shortcode = '';
    public $id = '';
    ...
    public function render()
    {
        ...
        if ($this->show_filter) {
            ...
            foreach ($all_categories as $category) {
                ...
                $html .= '<button class="btn btn-category bg-white ' . esc_attr($active_class) . '" id="category-' . esc_attr($category->name) . '" onclick="filterCategories(' . esc_js($category->cat_ID) . ')">' . esc_html($category->name) . '</button>';
            }
        }
        $html .= '</div>';
    ...
    {
        wp_enqueue_style('amb-full-width-gallery', plugins_url('masonary_gallery.css', __FILE__));
